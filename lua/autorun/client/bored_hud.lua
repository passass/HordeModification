...
function BoHU.GetHUDInfo()
	...
	if P:GetActiveWeapon() and P:GetActiveWeapon():IsValid() then
		...
		elseif PW.IsTFAWeapon then
			...
			if PW.GetHUDData then
				local HD = PW:GetHUDData()
				info.wp_firemode	= HD.mode or info.wp_firemode
				info.wp_clip1		= HD.clip or info.wp_clip1
				info.wp_ammo1		= HD.ammo or info.wp_ammo1
				info.wp_maxclip1	= HD.maxclip or info.wp_maxclip1
				info.wp_clipextra1	= HD.clipextra or info.wp_clipextra1
				info.wp_ammoname	= HD.ammoname or info.wp_ammoname
			end
