...
function entmeta:Horde_AddDebuffBuildup(debuff, buildup, inflictor, pos)
    ...
    if self:IsPlayer() then
		...
		if cancallhook then
            hook.Run("Horde_OnEnemyDebuffApplyPost", self, debuff, bonus, inflictor)
        end
    end
    if self.Horde_Debuff_Buildup[debuff] < 100 then return end
    ...
    if self:IsPlayer() then
		...
		hook.Run("Horde_PostPlayerDebuffApply", self, debuff)
    else
		hook.Run("Horde_PreEnemyDebuffApply", self, self.Horde_Debuff_Active)
        local duration = 5
