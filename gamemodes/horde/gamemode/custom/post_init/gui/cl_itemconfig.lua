...
local all_editors = {}
local editors_create_functions = {}
local function create_levels_editors(params)
    local name, height, panel, modify_tab =
    params.name,params.height,params.panel,all_editors.modify_tab
    local checkbox = vgui.Create("DCheckBoxLabel", panel)
    checkbox:SetSize(75, 75)
    checkbox:SetPos(5, 50)
    checkbox:SetText("Various Conditions?")
    checkbox:SetTextColor(Color(0,0,0))
    checkbox:SetChecked(not not params.varconds)
    local editors = {
        panel=panel,
        checkbox=checkbox,
        inputs = {}
    ...
    local function resetpanel()
        ...
        label:Dock(LEFT)
        label:SetWide(80)
        local new_editors = editors_create_functions.create_levels_editors(params)
        table.CopyFromTo( new_editors, editors )
        table.CopyFromTo( new_editors, all_editors.level_editors )
        all_editors.entity_properties_panel:SizeToChildren( false, true )
        all_editors.entity_properties_panel:SizeToContents()
    end
    checkbox.OnChange = function(self, bval)
        params.varconds = bval
        resetpanel()
    end
    
    local classes = table.GetKeys(HORDE.classes)
    local start_pos = 80
    local start_pos_2 = 80
    if checkbox:GetChecked() then
        if !params.how_times then
            params.how_times = 1
        end
        local add_btn = vgui.Create("DButton", panel)
        add_btn:SetText("+")
        add_btn:SetSize(15, 15)
        add_btn:SetPos(5, 30)
        add_btn.DoClick = function()
            params.how_times = params.how_times + 1
            resetpanel()
        end

        local remove_btn = vgui.Create("DButton", panel)
        remove_btn:SetText("-")
        remove_btn:SetSize(15, 15)
        remove_btn:SetPos(25, 30)
        remove_btn.DoClick = function()
            if params.how_times <= 1 then return end
            params.how_times = params.how_times - 1
            resetpanel()
        end
        
        for j=0, (params.how_times - 1) do
            start_pos = 80
            start_pos_2 = 80
            local height_for_table = 35 + 110 * j
            local allowed_classes = vgui.Create("DComboBox", panel)
            allowed_classes:SetSize(80, 30)
            allowed_classes:DockPadding(10, 10, 10, 10)
            allowed_classes:SetPos(start_pos, height_for_table - 35)
            function allowed_classes:analyze_classes()
                for _, editor in pairs(all_editors.whitelist_editors) do
                    if editor:GetChecked() then
                        self:AddChoice(editor:GetText())
                    end
                end
            end
            allowed_classes:analyze_classes()
            table.insert(editors.inputs, {class_choicer=allowed_classes, editors={}})
            for i, class in pairs(classes) do
                ...
                if i <= 5 then
                    editor:SetPos(start_pos, height_for_table)
                    editor_label:SetPos(start_pos + 30, height_for_table)
                    start_pos = start_pos + 75
                else
                    editor:SetPos(start_pos_2, height_for_table + 40)
                    editor_label:SetPos(start_pos_2 + 30, height_for_table + 40)
                    start_pos_2 = start_pos_2 + 75
                end
                table.insert(editors.inputs[#editors.inputs].editors, {editor=editor, label=editor_label})
            end
            if height_for_table + 40 > height then
                panel:SetSize(modify_tab:GetWide(), height_for_table + 100)
            end
        end
    else 
        for i, class in pairs(classes) do
            ...
            table.insert(editors.inputs, {editor=editor, label=editor_label})
        end
    end
    ...
    label:SetPos(80, h - 20)
    label:SetWide(200)

    return editors
end
editors_create_functions.create_levels_editors = create_levels_editors

function PANEL:Init()
    table.Empty(all_editors)
    ...
    all_editors.entity_properties_panel = entity_properties_panel
    all_editors.modify_tab = modify_tab
    ...
    local function create_property_editor(name, height, cat_panel, categories)
        ...
        elseif name == "whitelist" then
            ...
            local i = 1
            local classes = table.GetKeys(HORDE.classes)
            local function onchangecheckbox()
                for i, editor in pairs(level_editors.inputs) do
                    editor.class_choicer:analyze_classes()
                end
            end
            for _, class in pairs(classes) do
                ...
                editor.OnChange = onchangecheckbox
        ...
        elseif name == "levels" then
            return editors_create_functions.create_levels_editors({name=name, height=height, cat_panel=cat_panel, categories=categories, panel=panel})
    ...
    whitelist_editors = create_property_editor("whitelist", 40 * 2, entity_properties_panel)
    all_editors.whitelist_editors = whitelist_editors
    ...
    all_editors.level_editors = level_editors
    ...
    save_btn.DoClick = function ()
        ...
        if varconds then
            ...
            for _, data in pairs(level_editors.inputs) do
                local class = data.class_choicer:GetSelected()
                if !class then continue end
                levels[class] = {}
                for _, editor in pairs(data.editors) do
                    if editor.editor:GetInt() and editor.editor:GetInt() > 0 then
                        levels[class][editor.label:GetText()] = editor.editor:GetInt()
                    end
                end
            end
        else
            for _, editor in pairs(level_editors.inputs) do
                if editor.editor:GetInt() and editor.editor:GetInt() > 0 then
    ...
    item_list.OnClickLine = function(parent, line, selected)
        ...
        menu:AddOption("Modify", function()
            ...
            local levels = item.levels and table.Copy(item.levels) or {}
            local panel = level_editors.panel
            local params = {
                name = "levels",
                panel = panel,
                height = 150,
                how_times = table.Count(levels) - (levels.VariousConditions and 1 or 0),
                varconds = not not levels.VariousConditions,
            }
            levels.VariousConditions = nil
            panel:Clear()
            panel:SetSize(modify_tab:GetWide(), 150)
            local label = vgui.Create("DLabel", panel)
            ...
            table.CopyFromTo( editors_create_functions.create_levels_editors(params), level_editors )
            all_editors.entity_properties_panel:SizeToChildren( false, true )
            all_editors.entity_properties_panel:SizeToContents()
            

            if params.varconds then
                local onlykeys_levels = table.GetKeys(levels)

                for i, data in pairs(level_editors.inputs) do

                    local key = onlykeys_levels[i]
                    for i=1, 999 do
                        local key_2 = data.class_choicer:GetOptionText( i )
                        if !key_2 then break end
                        if key == data.class_choicer:GetOptionText( i ) then
                            data.class_choicer:ChooseOptionID(i)
                        end
                    end
                    
                    for _, editor in pairs(data.editors) do
                        editor.editor:SetValue(levels[key] and levels[key][editor.label:GetText()] or "")
                    end
                end
            else
                for _, editor in pairs(level_editors.inputs) do
                    editor.editor:SetValue(levels[editor.label:GetText()] or "")
                end
            end
