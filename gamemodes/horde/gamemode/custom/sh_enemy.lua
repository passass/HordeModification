...
function HORDE:GetDefaultEnemiesData()
    ...
    for round = 1, round_counts do
        for _, npcdata in pairs(news_npcs) do
            if istable(npcdata.wave) and (npcdata.wave[round_counts][1] <= round and round <= npcdata.wave[round_counts][2]) or !istable(npcdata.wave) and round >= npcdata.wave then
                HORDE:CreateEnemy(
                    npcdata.name,
                    npcdata.class,
                    npcdata.weight,
