...
function ENT:ColdAttack(delay, timerindex)
	...
	timer.Create("Horde_GammaGonome_ColdAttack_"  .. self:EntIndex() .. "_" .. timerindex, delay, 1, function()
		if not self:IsValid() then return end
        local rand = VectorRand()
        rand.z = 0
		local pos = self:GetPos() + dir
...
function ENT:Horde_StartTimeStop() 
	if !self.Horde_TimeStoped then 
		...
		if self.NextBlastTime then
			self.NextBlastTime = self.NextBlastTime + HORDE.TimeStop_TimeEnough()
		end
		local timername = "Horde_GammaGonome_ColdAttack_"  .. self:EntIndex() .. "_"
		local id = self:GetCreationID()
                timer.Pause("Horde_GammaUnInvis" .. id)
		for i=1, 25 * 12 do 
			timer.Pause(timername .. i)
		end
	end
end
...
function ENT:Horde_EndTimeStop() 
	if self.Horde_TimeStoped then 
		...
		local timername = "Horde_GammaGonome_ColdAttack_"  .. self:EntIndex() .. "_"
		local id = self:GetCreationID()
                timer.UnPause("Horde_GammaUnInvis" .. id)
		for i=1, 20 * 13 do 
			timer.UnPause(timername .. i)
		end
	end
end
	...
	if self.Critical then
        ...
        if EnemyDistance < 800 then
            if CurTime() > self.NextBlastTime then
                ...
                --[[for i = 1, 25 do
                    self:ColdAttack(3 + i * math.random() / 5, i)
                    self:ColdAttack(3 + i * math.random() / 5, i + 25)]]
				local p = math.random()
				local p2 = math.random()
				-- Cross shape blast
				for i = 1, 20 do
					self:ColdAttack(2, self:GetForward() * i * 100, i + 20 * 1)
					self:ColdAttack(2, -self:GetForward() * i * 100, i, i + 20 * 2)
					self:ColdAttack(2, self:GetRight() * i * 100, i, i + 20 * 3)
					self:ColdAttack(2, -self:GetRight() * i * 100, i, i + 20 * 4)

					local k1 = self:GetForward() + self:GetRight()
					k1:Normalize()
					local k2 = self:GetForward() - self:GetRight()
					k2:Normalize()
					self:ColdAttack(4, k1 * i * 100, i + 20 * 5)
					self:ColdAttack(4, -k1 * i * 100, i + 20 * 6)
					self:ColdAttack(4, k2 * i * 100, i + 20 * 7)
					self:ColdAttack(4, -k2 * i * 100, i + 20 * 8)

					if p <= 0.5 then
						if p2 <= 0.5 then
							self:ColdAttack(6, self:GetForward() * i * 100, i + 20 * 9)
							self:ColdAttack(6, -self:GetForward() * i * 100, i + 20 * 10)
							self:ColdAttack(6, self:GetRight() * i * 100, i + 20 * 11)
							self:ColdAttack(6, -self:GetRight() * i * 100, i + 20 * 12)
						else
							self:ColdAttack(6, k1 * i * 100, i + 20 * 9)
							self:ColdAttack(6, -k1 * i * 100, i + 20 * 10)
							self:ColdAttack(6, k2 * i * 100, i + 20 * 11)
							self:ColdAttack(6, -k2 * i * 100, i + 20 * 12)
						end
					end
                end
