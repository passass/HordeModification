...
function ENT:Horde_StartTimeStop() 
	if !self.Horde_TimeStoped then 
		...
		if self.Gonarch_NextBirthT then
			self.Gonarch_NextBirthT = self.Gonarch_NextBirthT + HORDE.TimeStop_TimeEnough()
		end
	end
end
...
function ENT:CustomOnThink_AIEnabled()
	...
	if self.Critical and self.Dead == false && IsValid(self:GetEnemy()) && self.PlayingAttackAnimation == false && CurTime() > self.Gonarch_NextBirthT && self.Gonarch_NumBabies < self.Gonarch_BabyLimit then
		...
		for i = 1, 3 do
			local bCrab = ents.Create("npc_vj_hordeext_headcrab_baby")
			if i == 1 then
