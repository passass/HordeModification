...
function ENT:Horde_StartTimeStop() 
	if !self.Horde_TimeStoped then 
		...
		local timeenough = HORDE.TimeStop_TimeEnough()
		if self.NextFlashTime then
			self.NextFlashTime = self.NextFlashTime + timeenough
		end
		if self.Garg_NextAbleToFlameT then 
			self.Garg_NextAbleToFlameT = self.Garg_NextAbleToFlameT + timeenough
		end
		timer.Pause("Horde_XenDestroyerUnit_explosion_" .. self:EntIndex())
	end
end
...
function ENT:Horde_EndTimeStop() 
	if self.Horde_TimeStoped then 
		...
		timer.UnPause("Horde_XenDestroyerUnit_explosion_" .. self:EntIndex())
	end
end
...
function ENT:CustomOnThink()
	...
	if self.Critical then
		...
		if self.NextFlashTime < CurTime() then
			...
			timer.Create("Horde_XenDestroyerUnit_explosion_" .. self:EntIndex(), 2, 1, function ()
